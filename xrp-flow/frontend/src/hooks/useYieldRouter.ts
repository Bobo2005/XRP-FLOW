...
export interface GasEstimate {
  gasUnits: bigint;
  costNative: bigint;
  formattedCost: string; // Formatted in standard units (e.g., C2FLR)
}

/**
 * Hook for interacting with the YieldRouter contract.
 * Provides functions for reading user data, writing transactions, and estimating gas.
 */
export function useYieldRouter() {
  const { address, isConnected } = useAccount();
  const chainId = useChainId();
  ...
  const readUserDeposit = useCallback(async () => {
    if (!enabled || !publicClient || !address) return { amount: 0n, timestamp: 0n };
    try {
      const result = await publicClient.readContract({
        address: CONTRACTS.yieldRouter.address,
        abi: CONTRACTS.yieldRouter.abi,
      ...
      if (Array.isArray(result) && result.length >= 2) {
        return { amount: BigInt(result[0]), timestamp: BigInt(result[1]) };
      }
    } catch (error) {
      console.error("[useYieldRouter] readUserDeposit failed", { error });
    }
  ...
  const readUserVenue = useCallback(async () => {
    if (!enabled || !publicClient || !address) return 0;
    try {
      const result = await publicClient.readContract({
        address: CONTRACTS.yieldRouter.address,
        abi: CONTRACTS.yieldRouter.abi,
  ...
  const readMorphoMarketParams = useCallback(async () => {
    if (!enabled || !publicClient) return null;
    try {
      const result = await publicClient.readContract({
        address: CONTRACTS.yieldRouter.address,
        abi: CONTRACTS.yieldRouter.abi,
  ...
  const readApyRates = useCallback(async () => {
    if (!enabled || !publicClient) return { kineticAPY: 0, morphoAPY: 0 };
    try {
      const [kineticResult, morphoResult] = await Promise.all([
        publicClient.readContract({
          address: CONTRACTS.yieldRouter.address,
      ...
      const extractValue = (val: unknown): bigint => {
        if (typeof val === "bigint") return val;
        if (Array.isArray(val) && val.length > 0 && typeof val[0] === "bigint") return val[0];
        return 0n;
      };
  ...
  const readAllowance = useCallback(async () => {
    if (!enabled || !publicClient || !address) return 0n;
    try {
      const result = await publicClient.readContract({
        address: CONTRACTS.fxrp.address,
        abi: CONTRACTS.fxrp.abi,
  ...
  const calculateGasCost = async (gasUnits: bigint): Promise<GasEstimate> => {
    if (!publicClient) throw new Error("Public client not available");
    const gasPrice = await publicClient.getGasPrice();
    const costNative = gasUnits * gasPrice;
    return {
      gasUnits,
      costNative,
      formattedCost: formatUnits(costNative, 18),
    };
  };

  const estimateApproveGas = useCallback(async (amount: bigint) => {
    if (!enabled || !publicClient || !address) return null;
    try {
      ...
      return await calculateGasCost(gas);
    } catch (error) {
      console.warn("Gas estimation failed for approve", error);
      return null;
    }
  }, [enabled, publicClient, address]);

  const estimateDepositGas = useCallback(async (amount: bigint) => {
    if (!enabled || !publicClient || !address) return null;
    try {
      const gas = await publicClient.estimateContractGas({
        address: CONTRACTS.yieldRouter.address,
      ...
      console.warn("Gas estimation failed for deposit", error);
      return null;
    }
  }, [enabled, publicClient, address]);

  const estimateDepositToVenueGas = useCallback(async (amount: bigint, venue: 0 | 1) => {
    if (!enabled || !publicClient || !address) return null;
    try {
      const gas = await publicClient.estimateContractGas({
        address: CONTRACTS.yieldRouter.address,
      ...
      console.warn("Gas estimation failed for depositToVenue", error);
      return null;
    }
  }, [enabled, publicClient, address]);

  const estimateWithdrawGas = useCallback(async (amount: bigint) => {
    if (!enabled || !publicClient || !address) return null;
    try {
      const gas = await publicClient.estimateContractGas({
        address: CONTRACTS.yieldRouter.address,
      ...
      console.warn("Gas estimation failed for withdraw", error);
      return null;
    }
  }, [enabled, publicClient, address]);

  const estimateRebalanceGas = useCallback(async () => {
    if (!enabled || !publicClient || !address) return null;
    try {
      const gas = await publicClient.estimateContractGas({
        address: CONTRACTS.yieldRouter.address,
      ...
      console.warn("Gas estimation failed for rebalance", error);
      return null;
    }
  }, [enabled, publicClient, address]);
  ...
  const approve = useCallback(async (amount: bigint) => {
    if (!enabled || !publicClient) throw new Error("Wallet not connected or network mismatch");
    try {
      const hash = await writeContractAsync({
        address: CONTRACTS.fxrp.address,
        abi: CONTRACTS.fxrp.abi,
      ...
      if (!isUserRejectedError(error)) console.error("[useYieldRouter] approve failed", error);
      throw error;
    }
  }, [enabled, publicClient, writeContractAsync]);
  ...
  const deposit = useCallback(async (amount: bigint) => {
    ...
    try {
      ...
      if (!isUserRejectedError(error)) console.error("[useYieldRouter] deposit failed", error);
      throw error;
    }
  }, [enabled, publicClient, writeContractAsync]);

  const depositToVenue = useCallback(async (amount: bigint, venue: 0 | 1) => {
    if (!enabled || !publicClient) throw new Error("Wallet not connected or network mismatch");
    try {
      const hash = await writeContractAsync({
        address: CONTRACTS.yieldRouter.address,
      ...
      if (!isUserRejectedError(error)) console.error("[useYieldRouter] depositToVenue failed", error);
      throw error;
    }
  }, [enabled, publicClient, writeContractAsync]);
  ...
  const withdraw = useCallback(async (amount: bigint) => {
    ...
    try {
      ...
      if (!isUserRejectedError(error)) console.error("[useYieldRouter] withdraw failed", error);
      throw error;
    }
  }, [enabled, publicClient, writeContractAsync]);
  ...
  const rebalance = useCallback(async () => {
    ...
    try {
      ...
      if (!isUserRejectedError(error)) console.error("[useYieldRouter] rebalance failed", error);
      throw error;
    }
  }, [enabled, publicClient, writeContractAsync]);
